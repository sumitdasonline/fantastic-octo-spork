...
export class TrengoAPI {
  ...
  constructor () {
    if (loginEmail === '' || loginPassword === '') {
      throw new Error('Please enter a valid username and password into cypress.env.json file')
    } else {
      const body = {
        email: loginEmail,
  ...
  customChannelMessage (username: string, name: string, message: string): void {
  ...
  deleteResource (resourceName: string, resourceId: string): void {
    const options = {
      method: 'DELETE',
